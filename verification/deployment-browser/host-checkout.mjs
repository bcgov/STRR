...
import { observeApplication, paySandboxCard, verifyPaidApplication } from './payment-helpers.mjs'

export async function fillHostForm(page, result) {
  ...
  await owner.getByTestId('phone-number').fill('')
...
export async function submitHostPayment(page, result, card) {
  ...
  const [response] = await Promise.all([
    page.waitForResponse(response => new URL(response.url()).hostname ===
      ...
      /^\/applications(?:\/\d+)?$/.test(new URL(response.url()).pathname) &&
      ['POST', 'PUT'].includes(response.request().method()), { timeout: 60000 }),
