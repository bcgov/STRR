...
export async function paySandboxCard(page, result, card, expectedAmount) {
  ...
  await writeFile('results/payment-checkpoint.json', JSON.stringify({
    runId: process.env.GITHUB_RUN_ID, applicationNumber: result.applicationNumber,
    invoiceId: result.invoiceId, amount, gatewayTestModeVerified: true,
    submissionAboutToStartAt: result.cardSubmittedAt
  }, null, 2) + '\n')
...
export async function verifyPaidApplication(page, result, dashboard, pending) {
  ...
  expect(result.receipt.result).toBe('passed')
