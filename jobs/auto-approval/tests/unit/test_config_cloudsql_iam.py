...
def _reload_config(monkeypatch, **env):
    ...
    from auto_approval import config as config_module
...
def test_production_config_uses_cloudsql_iam_connector(monkeypatch):
    ...
    config_module = _reload_config(
        ...
        DATABASE_USERNAME="sa-job@bcrbk9-prod.iam",
    )
...
def test_deployed_job_config_requires_cloudsql_iam_env(monkeypatch):
    with pytest.raises(
        ...
        _reload_config(monkeypatch, CLOUD_RUN_JOB="strr-auto-approval-prod")
