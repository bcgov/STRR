...
export const useHostApplicationFee = () => {
   ...
   */
  const fetchStrrFees = async (isRenewal = false) => {
    const [fee1, fee2, fee3] = await Promise.all([
      getFee(StrrFeeEntityType.STRR, isRenewal ? HostRenewalFeeCode.ONSITE : StrrFeeCode.STR_HOST_1),
      getFee(StrrFeeEntityType.STRR, isRenewal ? HostRenewalFeeCode.OFFSITE : StrrFeeCode.STR_HOST_2),
      getFee(StrrFeeEntityType.STRR, isRenewal ? HostRenewalFeeCode.BED_AND_BREAKFAST : StrrFeeCode.STR_HOST_3)
    ])

    APPLICATION_FEE_MATRIX = {
      [PropertyType.SINGLE_FAMILY_HOME]: {
        [RentalUnitSetupOption.DIFFERENT_PROPERTY]: fee2,
        [RentalUnitSetupOption.SEPARATE_UNIT_SAME_PROPERTY]: fee2,
        [RentalUnitSetupOption.PRIMARY_RESIDENCE_OR_SHARED_SPACE]: fee1
      },
      ...
      [PropertyType.BED_AND_BREAKFAST]: {
        [RentalUnitSetupOption.DIFFERENT_PROPERTY]: fee3,
        [RentalUnitSetupOption.SEPARATE_UNIT_SAME_PROPERTY]: fee3,
        [RentalUnitSetupOption.PRIMARY_RESIDENCE_OR_SHARED_SPACE]: fee3
      },
